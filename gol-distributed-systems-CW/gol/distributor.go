...
func initClient() {
	server := flag.String("server", "172.31.36.83:8030", "IP:port string to connect to as server")
	flag.Parse()
	var err error
...
func runNodes(p Params, world [][]byte, c distributorChannels, quit <-chan struct{}) [][]byte {
	...
	case <-quit:
		fmt.Println("Quit signal received: returning input World!")
		return world
...
func notifyServerOfQuit() {
	...
	fmt.Println("notifyServerOfQuit Called")
	...
	fmt.Println("notifyServerOfQuit Done")
}
...
func quitChannels(c distributorChannels, turn int) {
	fmt.Println("quitChannels Called")
...
func distributor(p Params, c distributorChannels) {
	...
	fmt.Println("notifyinf server of quit")
	notifyServerOfQuit()
	fmt.Println("hii")
